...
def f(csv_file,displayNum):
    f = pd.read_csv(csv_file)
    list_score = f[['タイトル', 'HYPER スコア', 'ANOTHER スコア', 'LEGGENDARIA スコア']].values.tolist()

    dict = create_dict()
    Theme_names = ['NOTES','PEAK','SCRATCH','SOF-LAN','CHARGE','CHORD']
    dict_score = []
    for score in list_score:
        dict_score.append({'曲名':score[0],'sph':score[1],'spa':score[2],'spl':score[3]})

    que_score = {key:[{'Score':0}] for key in Theme_names}

    for row in dict_score:
        for diff in ['sph','spa','spl']:
            for Theme in Theme_names:
                S = calc_score(dict,row['曲名'],diff,float(row[diff]),Theme)
                if S > que_score[Theme][-1]['Score']:
                    que_score[Theme].append({'Score':S,'Name':row['曲名'],'diff':diff})
                    que_score[Theme] = sorted(que_score[Theme], key=lambda x:x['Score'], reverse=True)
                if len(que_score[Theme])>displayNum:
                    que_score[Theme].pop()

    que_score['SOF_LAN'] = que_score['SOF-LAN']
    del que_score['SOF-LAN']
